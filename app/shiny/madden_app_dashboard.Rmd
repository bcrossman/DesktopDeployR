...
DT::renderDataTable({
  ...
  WR_chose_attr <-    isolate(input$attributes_target_2_WR)
  QB_chose_attr <-    isolate(input$attributes_target_2_QB)
  C_chose_attr <-     isolate(input$attributes_target_2_C)
  HB_chose_attr <-    isolate(input$attributes_target_2_HB)
  FB_chose_attr <-    isolate(input$attributes_target_2_FB)
  CB_chose_attr <-    isolate(input$attributes_target_2_CB)
  RE_chose_attr <-    isolate(input$attributes_target_2_RE)
  RG_chose_attr <-    isolate(input$attributes_target_2_RG)
  RT_chose_attr <-    isolate(input$attributes_target_2_RT)
  LE_chose_attr <-    isolate(input$attributes_target_2_LE)
  LG_chose_attr <-    isolate(input$attributes_target_2_LG)
  LT_chose_attr <-    isolate(input$attributes_target_2_LT)
  DT_chose_attr <-    isolate(input$attributes_target_2_DT)
  LOLB_chose_attr <-  isolate(input$attributes_target_2_LOLB)
  ROLB_chose_attr <-  isolate(input$attributes_target_2_ROLB)
  SS_chose_attr <-    isolate(input$attributes_target_2_SS)
  FS_chose_attr <-    isolate(input$attributes_target_2_FS)
  TE_chose_attr <-    isolate(input$attributes_target_2_TE)
  MLB_chose_attr <-   isolate(input$attributes_target_2_MLB)
  P_chose_attr <-     isolate(input$attributes_target_2_P)
  K_chose_attr <-     isolate(input$attributes_target_2_K)
...
selectInput(inputId = "attributes_target_team_overview",
            label = "What Stat do you want to review:",
            choices = names(player_data %>% select(-teamId,-firstName, -team,  -lastName,-position)),
            selected = "playerBestOvr",
            multiple = FALSE
)
...
renderPlot({
  ...
  characteristic <- input$attributes_target_team_overview
  # characteristic <- "playerBestOvr"
  ...
  player_data_cut %>% 
    ...
    summarise(rating = mean(!!sym(characteristic)),
              max_rating = max(!!sym(characteristic)),
              min_rating = min(!!sym(characteristic))) %>%
    ...
    ggplot(aes(team, rating, ymin=min_rating, ymax=max_rating, 
               ...
               color = team)) +
    geom_pointrange()+
    facet_wrap(~group_position, scales = "free_x", nrow = 1) +
    coord_flip()+
    # scale_x_reordered() +
    # scale_y_continuous(limits=c(60,90),oob = rescale_none)
    labs(y = paste("Average",characteristic,"Rating of Starters by Position")) +
